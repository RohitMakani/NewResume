...
async def generate_ai_suggestions(resume_text: str, job_description: Optional[str] = None) -> List[EnhancementSuggestion]:
    ...
    try:
        ...
        response = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are an expert resume reviewer and career counselor with 10+ years of experience in HR and recruiting."},
...
async def generate_complete_resume(resume_text: str, job_description: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        response = groq_client.chat.completions.create(
            model="llama3-8b-8192",
            messages=[
                {"role": "system", "content": "You are an expert resume writer who creates ATS-optimized, compelling resumes that get interviews."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.4,
            max_tokens=3000
...
async def get_job_role_recommendations(resume_text: str) -> List[str]:
    ...
    try:
        ...
        response = groq_client.chat.completions.create(
            model="llama3-8b-8192",
            messages=[
                {"role": "system", "content": "You are a career counselor who recommends suitable job roles based on candidate profiles."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.5,
            max_tokens=500
